...
declare -A online_music=(
  ...
  ["Quran Badr-turki"]="https://www.youtube.com/@B_m_turki/featured"
...
notification() {
  notify-send -u -t 2000 normal -i "$iDIR/music.png" "Playing: $@"
}
...
play_local_music() {
  ...
  choice=$(printf "%s\n" "${filenames[@]}" | rofi -i -dmenu -theme "$theme" -config ~/.config/rofi/config-rofi-Beats.rasi -p "Local Music")
...
play_online_music() {
  choice=$(printf "%s\n" "${!online_music[@]}" | rofi -theme "$theme" -i -dmenu -config ~/.config/rofi/config-rofi-Beats.rasi -p "Online Music")
...
pkill mpv && notify-send -u low -t 2000 -i "$iDIR/music.png" "Music stopped" || {

# Prompt the user to choose between local and online music
user_choice=$(printf "Play from Online Stations\nPlay from Music Folder\nShuffle Play from Music Folder" | rofi -theme "$theme" -dmenu -config ~/.config/rofi/config-rofi-Beats-menu.rasi -p "Select music source")
