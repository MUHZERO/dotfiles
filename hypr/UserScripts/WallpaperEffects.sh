...
no_effects() {
    ...
    notify-send -u low -t 2000 -i "$iDIR/bell.png" "No wallpaper effects"
...
main() {
    ...
    # rofi
    choice=$(echo -e "$options" | rofi -i -theme "$theme" -dmenu -config ~/.config/rofi/config-wallpaper-effect.rasi) 
    if [ ! -z "$choice" ]; then
        ...
        if [[ "${effects[$choice]+exists}" ]]; then
            if [ "$choice" == "No Effects" ]; then
                ...
                notify-send -u normal t- 2000 -i "$iDIR/bell.png" "Applying $choice effects"
                eval "${effects[$choice]}"
                ...
                # Refresh rofi, waybar, wallust palettes
                "${SCRIPTSDIR}/Refresh.sh"
                notify-send -u low -t 2000 -i "$iDIR/bell.png" "$choice effects applied"
            fi
        else
